...
To update the database use this command from the Web project folder (replace `ExpenseManagement` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../ExpenseManagement.Infrastructure/ExpenseManagement.Infrastructure.csproj -s ExpenseManagement.Web.csproj
```
